...
int poet_register(json_value *json, socket_t *socket) {
    ...
    pk_64base_len = pk_json->u.string.length;
    ...
    sign_64base_len = pk_json->u.string.length;

    size_t buff_len;
    buff = decode_64base(pk_64base, pk_64base_len, &buff_len);
    ...
    sprintf(msg, R"({"sgxmax" : %lu, "sgxt_lower": %lu, "node_id" : %u})", sgxmax, sgxt_lowerbound, current_id++);
    ...
    sgxt = json->u.object.values[0].value->u.integer;
    printf("SGXt is received and checking the validity.\n");

    valid = (sgxt_lowerbound <= sgxt && sgxt <= sgxmax);
